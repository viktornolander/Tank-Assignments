...
-- events
TankAssignments:RegisterEvent("PLAYER_ENTERING_WORLD")
...
function TankAssignments:OnEvent()
	if event == "PLAYER_ENTERING_WORLD" then
		--if arg1 == "TankAssignments" then
		print("|cffC79C6E TankAssignments 2.0|r: TankAssignments 2.0 Loaded!")
		print("|cffC79C6E TankAssignments 2.0|r: /ta - open/close TankAssignments")
		print("|cffC79C6E TankAssignments 2.0|r: /ta colors - turn chatcolors on/off")
		...
		if string.sub(arg1,1,15) == "TankAssignments" and UnitName("player") ~= arg4 then
			if string.sub(arg1,16,string.len(arg1)) == "Marks" then
				...
				for text in string.gfind(arg2,"%d%a+") do	
					...
					--print(mark..string.sub(text,2,string.len(text)))
...
function TankAssignments:ConfigMainFrame()
	...
	for n, class in pairs(TankAssignments.Classes) do	
		...
		classframe:SetScript("OnEnter", function() 
			--print("class "..classframe:GetName())
			local r,g,b=TankAssignments:GetClassColors(classframe:GetName(),"class")
			GameTooltip:SetOwner(classframe, "ANCHOR_TOPRIGHT");
			GameTooltip:SetText("|cffFFFFFFShow|r "..classframe:GetName(), r,g,b);
			GameTooltip:Show()
		end)
		classframe:SetScript("OnLeave", function() GameTooltip:Hide() end)
		classframe:SetScript("OnClick", function()
			--print("click")
			--if (arg1 == "LeftButton") then
			
				if TankAssignments_Settings[classframe:GetName()] == 1 then
					TankAssignments_Settings[classframe:GetName()] = 0
					classframe.Icon:SetVertexColor(0.5, 0.5, 0.5)
				else
					TankAssignments_Settings[classframe:GetName()] = 1
	...
	for i in pairs(TankAssignments.Marks) do
		...
		icon:SetScript("OnEnter", function()
			--print("icon")
			TankAssignments:OpenToolTip(icon:GetName())
		end)
...
function TankAssignments.Minimap:CreateMinimapIcon()
	...
	function self:OnMouseDown(arg1)
		--print("mousedown "..arg1)
		Moving = false;
		if (arg1 == "LeftButton") then 
			if TankAssignments:IsVisible() then 
				TankAssignments.ToolTip:Hide()
...
function TankAssignments:AddFrame(name)
	...
	if UnitIsVisible(unit) then frame.model:SetCamera(0) end 
	end)
	...
	frame:SetScript("OnClick", function()
		...
		--print("click addtank ")
			frame:Hide()
			TankAssignments:AddTank(frame:GetName(), TankAssignments.Settings["active"])
...
function TankAssignments:Slash(arg1)
	...
	elseif arg1 == "colors" then
		if not TankAssignments_Settings["usecolors"] then
			...
			print("|cffC79C6E TankAssignments 2.0|r: Using chat colors (note that some servers do not support this option)")
		else
			TankAssignments_Settings["usecolors"] = false
			print("|cffC79C6E TankAssignments 2.0|r: Chat colors turned off")		
		end
		]]
	else
		print("TankAssignments: There are no other commands")
	end
end
